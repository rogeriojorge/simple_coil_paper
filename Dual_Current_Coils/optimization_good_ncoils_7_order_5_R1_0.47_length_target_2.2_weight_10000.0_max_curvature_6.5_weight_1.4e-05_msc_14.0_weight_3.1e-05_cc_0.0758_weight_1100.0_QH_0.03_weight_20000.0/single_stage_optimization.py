...
order_coils = 5 # 4 #5
R1 = 0.47 # 0.69 # 0.67
LENGTH_THRESHOLD = 2.2 # 2.4 # 3.8
LENGTH_CON_WEIGHT = 0.0069 # 6.9e-3 # 5.1e-1
CURVATURE_THRESHOLD = 6.5 # 2.4e+1 # 6.5
CURVATURE_WEIGHT = 1.4e-5 # 1.2e-3 # 7.1e-3  # Weight for the curvature penalty in the objective function
MSC_THRESHOLD = 1.4e+1 # 6.8 # 5.6
MSC_WEIGHT = 3.1e-5 # 3.5e-6 # 2.4e-6  # Weight for the mean squared curvature penalty in the objective function
CC_THRESHOLD = 0.0758 # 0.089 # 0.11
CC_WEIGHT = 1.1e+3 # 7.7 # 2.7e+2
...
for iteration, max_mode in enumerate(max_mode_array):
    ...
    if optimize_stage_2:
        ...
        if mpi.proc0_world:
            res = minimize(fun_coils, dofs[:-number_vmec_dofs], jac=True, args=({'Nfeval': 0}), method='L-BFGS-B', options={'maxiter': MAXITER_stage_2, 'maxcor': 300}, tol=1e-8)
            print(res.message)
